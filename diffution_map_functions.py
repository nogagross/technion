...
def load_tsv_folder_as_subject_dict(folder: Union[str, Path],
                                    ...
                                    drop_first_row: bool,
                                    drop_first_col: bool,
                                    on_duplicate: str = "latest",MISSING_ROWS_CSV = None
                                    ) -> Union[Dict[Key, np.ndarray], Dict[Key, List[np.ndarray]]]:
    """
    טעינת נתונים כולל מחיקת שורות ועמודות לפי שמות.
    """
    # *** חובה: שניהם True כדי ששמות השורות יתיישרו נכון לנתונים ***
    if not (drop_first_row and drop_first_col):
        raise ValueError(
            "When removing rows/cols by names, please set IGNORE_FIRST_ROW=True and IGNORE_FIRST_COL=True.")
